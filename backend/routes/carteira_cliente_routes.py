...
@carteira_cliente_bp.route('/api/carteira-cliente/<int:carteira_id>/cliente', methods=['GET'])
@token_required
def get_cliente_by_carteira_id(carteira_id, current_user=None):
    """
    Retorna o nome do cliente baseado no ID da carteira.
    """
    try:
        connection = get_db_connection()
        if connection is None:
        ...
        # Buscar o cliente associado à carteira
        cursor.execute("""
            SELECT c.client_name, c.id as cliente_id
            FROM carteira_cliente cc
            JOIN client c ON cc.cliente_id = c.id
            WHERE cc.id = %s
        """, (carteira_id,))
        ...
        if request.user_role != 'Admin':
            cursor.execute("""
                SELECT id FROM client 
                WHERE id = %s AND user_id = %s
            """, (result['cliente_id'], request.user_id))
            if not cursor.fetchone():
                return jsonify({'error': 'Acesso negado a este cliente'}), 403
